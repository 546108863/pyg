			...
			<div class="cart-tbody" v-if="shopCartInfo.shopCartList.length">
				...
				<div class="item-list">
                    ...
                    autocomplete="off"
										:value="item.skuNum"
										@change="
											skuNumChange('changeSkuNum', $event.target.value, item,$event)
										"
									/>
									<button
										@click="
											skuNumChange('addSkuNum',1, item)
										"
									>
										+
									</button>
...
<script>
	...
	export default {
		...
		methods: {
			skuNumChange(type, disNum, sku,e) {
				switch (type) {
					case "addSkuNum":
						sku.skuNum++;
						break;
          ...
          case "changeSkuNum":
            ...
            if (!isNaN(disNum) && disNum >= 0) {
              e.target.value = parseInt(disNum);
              sku.skuNum = parseInt(disNum);
            } else {
          ...
          this.$store.dispatch("cart/changeSkuNum", { disNum, sku});	
			},
		},
		computed: {
